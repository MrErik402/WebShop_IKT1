...
export default async function AirRuhazat(){

    let AirRuhazat = []

    function PassData(data){
        data.map(info => {
            AirRuhazat.push(new Termek(info.id, info.nev, info.ar, info.leiras, info.kep));
        });
    ...
    try {
        await fetch("/Src/Javascript/Products/AirsoftRuhazat/products.json")
        .then(res => res.json())
