...
export default async function AirKiegeszito(){

    let AirKiegeszito = []

    function PassData(data){
        data.map(info => {
            AirKiegeszito.push(new Termek(info.id, info.nev, info.ar, info.leiras, info.kep));
        });
    ...
    try {
        await fetch("/Src/Javascript/Products/AirsoftKiegeszitok/products.json")
        .then(res => res.json())
